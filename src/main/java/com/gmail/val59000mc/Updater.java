...
public class Updater extends Thread implements Listener {
    ...
    public void onCommand(PlayerCommandPreprocessEvent e) {
        if (!e.getMessage().equalsIgnoreCase("/uhccore update")) { return; }
        e.setCancelled(true);
        ...
        if (gm.getGameState() == GameState.PLAYING || gm.getGameState() == GameState.DEATHMATCH) {
            player.sendMessage(
                    ChatColor.RED + "You can not update the plugin during games as it will restart your server.");
            return;
        }
    ...
    private void runVersionCheck() throws Exception {
        ...
        // URL(LATEST_RELEASE).openConnection();

        // // Add headers
        // connection.setRequestMethod("GET");
        // connection.addRequestProperty("Accept", "application/json");
        // connection.addRequestProperty("User-Agent", "UhcCore:"+
        // UhcCore.getPlugin().getDescription().getVersion());

        // connection.connect();

        // JsonParser jp = new JsonParser();
        // JsonObject root = jp.parse(new
        // InputStreamReader(connection.getInputStream())).getAsJsonObject();
        ...
        // // New version is available, register player join listener so we can notify
        // admins.
    ...
    private void sendUpdateMessage(CommandSender receiver) {
        ...
        receiver.sendMessage(ChatColor.DARK_GREEN + "[UhcCore] " + ChatColor.GREEN
                + "A new version of the UhcCore plugin is available!");
        ...
        receiver.sendMessage(ChatColor.DARK_RED.toString() + ChatColor.BOLD + "WARNING: " + ChatColor.RED
    ...
    public void runAutoUpdate() {
        ...
        if (!GameManager.getGameManager().getConfiguration().getEnableAutoUpdate()) { return; }
    ...
    private static class Version {
        ...
        public boolean equals(Version version) { return this.version.equals(version.version); }

        @Override
        public String toString() { return version; }

        private int getVersionNumber(int index) {
            if (versionNums.length > index) { return versionNums[index]; }
        ...
        private boolean isNewerThan(Version version) {
            if (equals(version)) { return false; }

            int numCount = versionNums.length;
            if (version.versionNums.length > numCount) { numCount = version.versionNums.length; }

            for (int i = 0; i < numCount; i++) {
                // This version is smaller than arg version so this is old
                if (getVersionNumber(i) < version.getVersionNumber(i)) { return false; }
                // This version is bigger than arg version so this is new
                if (getVersionNumber(i) > version.getVersionNumber(i)) { return true; }

                // If neither of those are true both numbers are equal and a later number in the
                // line will define age
            }
