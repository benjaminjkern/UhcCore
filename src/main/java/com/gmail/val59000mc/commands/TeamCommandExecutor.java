...
public class TeamCommandExecutor implements CommandExecutor {
    ...
    public TeamCommandExecutor(GameManager gameManager) { this.gameManager = gameManager; }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (subCommand.equals("list")) {
            for (UhcTeam team : gameManager.getTeamManager().getUhcTeams()) {
                player.sendMessage("Team " + team.getTeamName() + ":");
                for (UhcPlayer p : team.getPlayingMembers()) {
                    try {
                        player.sendMessage("  " + p.getPlayer().getName());
                    } catch (UhcPlayerNotOnlineException e) {
                        player.sendMessage("  \u00a7ePlayer not online");
                    }
                }
            }
            return true;
        ...
        if (gameManager.getGameState() != GameState.WAITING) { return true; }
