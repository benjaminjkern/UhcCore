...
public class EntityHider implements Listener {
    ...
    protected void removeEntity(Entity entity, boolean destroyed) {
        ...
        for (Map<Integer, Boolean> maps : observerEntityMap.rowMap().values()) { maps.remove(entityID); }
    }
    ...
    private Listener constructBukkit() {
        return new Listener() {
            @EventHandler
            public void onEntityDeath(EntityDeathEvent e) { removeEntity(e.getEntity(), true); }

            @EventHandler
            public void onChunkUnload(ChunkUnloadEvent e) {
                for (Entity entity : e.getChunk().getEntities()) { removeEntity(entity, false); }
            }

            @EventHandler
            public void onPlayerQuit(PlayerQuitEvent e) { removePlayer(e.getPlayer()); }
        };
    }
    ...
    private PacketAdapter constructProtocol(Plugin plugin) {
        return new PacketAdapter(plugin, ENTITY_PACKETS) {
            ...
            public void onPacketSending(PacketEvent event) {
                ...
                if (!isVisible(event.getPlayer(), entityID)) { event.setCancelled(true); }
            }
        };
    }
    ...
    public final boolean showEntity(Player observer, Entity entity) {
        ...
        // Resend packets
        if (manager != null && hiddenBefore) { manager.updateEntity(entity, Arrays.asList(observer)); }
        return hiddenBefore;
    }
