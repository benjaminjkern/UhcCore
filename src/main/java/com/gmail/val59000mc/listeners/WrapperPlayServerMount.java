...
public class WrapperPlayServerMount extends AbstractPacket {
    ...
    public WrapperPlayServerMount(PacketContainer packet) { super(packet, TYPE); }
    ...
    public int[] getPassengerIds() { return handle.getIntegerArrays().read(0); }

    public void setPassengerIds(int[] value) { handle.getIntegerArrays().write(0, value); }

    public List<Entity> getPassengers(PacketEvent event) { return getPassengers(event.getPlayer().getWorld()); }
    ...
    public List<Entity> getPassengers(World world) {
        ...
        for (int id : ids) {
            ...
            if (entity != null) { passengers.add(entity); }
        }
    ...
    public void setPassengers(List<Entity> value) {
        ...
        for (int i = 0; i < value.size(); i++) { array[i] = value.get(i).getEntityId(); }
