...
public class MapLoader {
	...
	public void generateChunks(final Environment env) {
		...
		Bukkit.getLogger().info("[UhcCore] Loading a total " + Math.floor(totalChunksToLoad) + " chunks, up to chunk ( "
				+ maxChunk + " , " + maxChunk + " )");
		...
		Bukkit.getScheduler().runTaskAsynchronously(UhcCore.getPlugin(), new Runnable() {
			...
			public void run() {
					...
					public void run() {
						...
						if (i <= maxChunk) {
							...
							Bukkit.getScheduler().scheduleSyncDelayedTask(UhcCore.getPlugin(),
									new RunnableWithParameter(i, j, nextRest), delayTask);
						} else {
							...
							if (env.equals(Environment.NORMAL) && gm.getConfiguration().getEnableNether()) {
								...
								GameManager.getGameManager().finishLoad("LOADCHUNKS");
							}
						}
					}
				}
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(UhcCore.getPlugin(),
						new RunnableWithParameter(-maxChunk, -maxChunk, restEveryTicks), 0);
