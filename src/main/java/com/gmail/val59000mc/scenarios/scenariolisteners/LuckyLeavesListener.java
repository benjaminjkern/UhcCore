...
public class LuckyLeavesListener extends ScenarioListener {
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (Math.random() * 200 <= 1) e.getBlock().getWorld().dropItem(e.getBlock().getLocation().add(.5, 0, .5),
                new ItemStack(Material.GOLDEN_APPLE));

        if (Math.random() * 2000 <= 1) e.getBlock().getWorld().dropItem(e.getBlock().getLocation().add(.5, 0, .5),
