...
public class PoliticsListener extends ScenarioListener {
    ...
    // List<String> scoreboardList = Arrays.asList("&fTime: &d%time%", "&fBorder:
    // &d%border%", "&fPlayers: &d%alive%",
    // "&fTeams left: &d%teamAlive%", "&fTeam: &d%teamColor%", "&fChain of
    // command:", "&d%chainOfCommand%",
    // "&fTeam size: &d%teammatesAlive%", "&fKills: &d%kills%", "&fScenarios:",
    // "&d%scenarios%", "",
    ...
    // scoreboardList = Arrays.asList("&fTime: &d%time%", "&fBorder: &d%border%",
    // "&fPlayers: &d%alive%",
    // "&fTeams left: &d%teamAlive%", "&fScenarios:", "&d%scenarios%", "", "&fPlayer
    // Rating:",
    // "&6%userScore%");
    ...
    // if (damager instanceof Projectile) damager = (Entity) ((Projectile)
    // damager).getShooter();
    // if (damager instanceof TNTPrimed) damager = ((TNTPrimed)
    // damager).getSource();
    ...
    // killer.getInventory().setContents(killed.getInventory().getContents());

    // // check if it was friendlyfire
    // if (uhcKiller.getTeam() != uhcKilled.getTeam()) {
    // GameManager gm = getGameManager();
    // gm.sendInfoToServer("KILL:" + uhcKiller.getName() + ":" + uhcKilled.getName()
    // + ":" + true, false);
    ...
    // if (teamBelow.size() == 1) {
    // uhcKiller.getTeam().sendMessage(killedName + " \u00a7fwas recruited by " +
    // uhcKiller.getDisplayName());
    // } else {
    // uhcKiller.getTeam().sendMessage(killedName + " \u00a7fand \u00a7d" +
    // (teamBelow.size() - 1)
    // + " \u00a7fothers were recruited by " + uhcKiller.getDisplayName());
    // }

    // killed.sendMessage("\u00a7fYou have joined the " +
    // uhcKiller.getTeam().getPrefix() + "\u00a7fteam!");
    // if (teamBelow.size() > 1) killed
    // .sendMessage("\u00a7fYou brought \u00a7d" + (teamBelow.size() - 1) + "
    // \u00a7fteammates with you");
    ...
    // player.getWorld().playSound(player.getLocation(), Sound.BLOCK_ANVIL_PLACE, 1,
    // 0);
    // player.getWorld().spawnParticle(Particle.BLOCK_CRACK,
    // killed.getLocation().clone().add(0, 1, 0),
    // 200, 0, 0, 0, Bukkit.createBlockData(Material.DIAMOND_BLOCK));
    // player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(
    // "\u00a7fYou have joined the " + uhcKiller.getTeam().getPrefix() +
    // "\u00a7fteam!"));
    // if (node != killedNode) player.sendMessage(killedName + " \u00a7fjoined the "
    // + uhcKiller.getTeam().getPrefix() + "\u00a7fteam and has brought you with
    // them!");
    ...
    // if (teamBelow.size() == 1) {
    // killedTeam.sendMessage(killedName + " \u00a7fhas joined team " +
    // uhcKilled.getTeam().getPrefix());
    // } else {
    // killedTeam.sendMessage(killedName + " \u00a7fand \u00a7d" + (teamBelow.size()
    // - 1)
    ...
    // getGameManager().broadcastInfoMessage(uhcKiller.getDisplayName() + "
    // committed treason!");
    // GameManager gm = getGameManager();
    // gm.sendInfoToServer("KILL:" + uhcKilled.getName() + ":" + uhcKiller.getName()
    // + ":" + true, false);
    // if (treason.contains(uhcKiller)) {
    // Bukkit.dispatchCommand(killer, "suicide");
    // } else {
    // killer.sendMessage(
    // UhcCore.PREFIX + " \u00a7cDon't do that again or you'll get kicked from the
    // game!");
    ...
    // killed.sendMessage("\u00a77\u00a7oYou were killed by \u00a7d" +
    // cause.substring(0, 1).toUpperCase()
    // + cause.substring(1).toLowerCase() + " \u00a77\u00a7oand got moved to the
    // bottom of the totem poll");

    // if (killedNode.child != null) {
    // uhcKilled.getTeam().sendMessage(killedName + " \u00a77\u00a7ohas been
    // demoted");
    // if (killedNode.isLeader()) uhcKilled.getTeam().sendMessage(
    // killedNode.child.player.getDisplayName() + " \u00a7ehas been promoted to team
    // leader!");
    // }
    ...
    // new TextComponent("\u00a7e\u00a7lYou have been promoted to rank
    // \u00a7f\u00a7l"
    ...
    // if (teamMate.getName().equals("YEUH-BOT") &&
    // teamMate.getState().equals(PlayerState.PLAYING)) {
    // Player matePlayer;
    // try {
    ...
    // killed.getActivePotionEffects().forEach(effect ->
    // killed.removePotionEffect(effect.getType()));
    // if (isActivated(Scenario.SUPERHEROES)) {
    // SuperHeroesListener.addHeroesEffect(uhcKilled,
    // SuperHeroesListener.getPower(uhcKilled), false);
    // }
    ...
    // }
    // if (Math.random() < 0.5
    // && (!isActivated(Scenario.NINESLOTS) ||
    ...
    // HashMap<Integer, ItemStack> a =
    // killer.getInventory().addItem(drops.stream().toArray(ItemStack[]::new));
    // drops.clear();
    // drops.addAll(a.values());
    // }

    // if (isActivated(Scenario.TIMEBOMB)) {
    // TimebombListener.TimebombThread timebombThread = new
    // TimebombListener.TimebombThread(drops,
    // killed.getLocation().getBlock().getLocation(), killed.getName(),
    // TimebombListener.delay);
    ...
    // .filter(uhcPlayer -> uhcPlayer != player).map(uhcPlayer ->
    // getPlayerNode(uhcPlayer))
