...
public class SkyHighListener extends ScenarioListener {
    ...
    public SkyHighListener() { taskId = -1; }

    @EventHandler
    public void onGameStarted(UhcStartedEvent e) {
        taskId = Bukkit.getScheduler().scheduleSyncDelayedTask(UhcCore.getPlugin(), new SkyHighThread(this),
                delay * TimeUtils.SECOND_TICKS);
    }
    ...
    public void onEnable() {
        ...
        if (getGameManager().getGameState() == GameState.PLAYING
            ...
            if (timeUntilFirstRun < 0) { timeUntilFirstRun = 0; }
            taskId = Bukkit.getScheduler().scheduleSyncDelayedTask(UhcCore.getPlugin(), new SkyHighThread(this),
                    timeUntilFirstRun);
        }
    }
    ...
    public static class SkyHighThread implements Runnable {
        ...
        public SkyHighThread(SkyHighListener listener) { this.listener = listener; }

        @Override
        public void run() {
            // damage players
            for (UhcPlayer uhcPlayer : GameManager.getGameManager().getPlayersManager().getAllPlayingPlayers()) {
            ...
            listener.taskId = Bukkit.getScheduler().scheduleSyncDelayedTask(UhcCore.getPlugin(), this,
                    listener.period * TimeUtils.SECOND_TICKS);
        }
