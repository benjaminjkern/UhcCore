...
public class VeinMinerListener extends ScenarioListener {
    ...
    private static final BlockFace[] BLOCK_FACES = new BlockFace[] { BlockFace.DOWN, BlockFace.UP, BlockFace.SOUTH,
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (!UniversalMaterial.isCorrectTool(block.getType(), player.getInventory().getItemInMainHand().getType())) {
            return;
        }
        ...
        player.getWorld().dropItem(player.getLocation().getBlock().getLocation().add(.5, .5, .5),
                vein.getDrops(getVeinMultiplier(vein.getDropType())));

        if (vein.getTotalXp() != 0) { UhcItems.spawnExtraXp(player.getLocation(), vein.getTotalXp()); }
    ...
    private int getVeinMultiplier(Material material) {
        int multiplier = 1;
        if (getScenarioManager().isActivated(Scenario.TRIPLEORES)) { multiplier *= 3; }
    ...
    private static class Vein {
        ...
        public void process() { getVeinBlocks(startBlock, startBlock.getType(), 2, 10); }

        public ItemStack getDrops() { return getDrops(1); }

        public ItemStack getDrops(int multiplier) {
            Material material = getDropType();
            if (material == null) return null;

            if (material == Material.LAPIS_ORE) { return UniversalMaterial.LAPIS_LAZULI.getStack(ores * multiplier); }

            return new ItemStack(material, ores * multiplier);
        }

        public int getTotalXp() { return getXpPerBlock() * ores; }
        ...
        private void getVeinBlocks(Block block, Material type, int i, int maxBlocks) {
            ...
            if (i > 0 && ores < 20) {
                for (BlockFace face : BLOCK_FACES) { getVeinBlocks(block.getRelative(face), type, i, maxBlocks - 1); }
            }
        }

        private Material getDropType() {
            if (startBlock.getType() == UniversalMaterial.NETHER_QUARTZ_ORE.getType()) { return Material.QUARTZ; }
        ...
        private int getXpPerBlock() {
            if (startBlock.getType() == UniversalMaterial.NETHER_QUARTZ_ORE.getType()) { return 3; }
