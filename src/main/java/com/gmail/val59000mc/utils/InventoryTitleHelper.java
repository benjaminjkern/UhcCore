...
/**
 * A inventory helper that allows you to change the title of the current opened
 * inventory.
 */
public class InventoryTitleHelper {
    ...
    private static void sendInventoryTitle0(Player player, String title) throws Exception {
        ...
        if (m_Player_GetHandle == null) { m_Player_GetHandle = player.getClass().getMethod("getHandle"); }
        ...
        if (f_Container_windowId == null) { f_Container_windowId = nms_Container.getClass().getField("windowId"); }
        ...
        if (version.startsWith("v1_5_") || version.startsWith("v1_6_")) {
            sendPacket15a16a17(nms_PlayerConnection, nms_EntityPlayer, nms_Container, windowId, inventory, title,
                    false);
    ...
    private static void sendPacket15a16a17(Object nms_playerConnection, Object nms_EntityPlayer, Object nms_Container,
            int windowId, Inventory inventory, String title, boolean flag) throws Exception {
        if (c_PacketOpenWindow == null) {
            if (flag) {
                c_PacketOpenWindow = findNmsClass("PacketPlayOutOpenWindow").getConstructor(int.class, int.class,
                        String.class, int.class, boolean.class);
            } else {
                c_PacketOpenWindow = findNmsClass("Packet100OpenWindow").getConstructor(int.class, int.class,
                        String.class, int.class, boolean.class);
            }
        }
        ...
        if (m_EntityPlayer_updateInventory == null) {
            m_EntityPlayer_updateInventory = nms_EntityPlayer.getClass().getMethod("updateInventory",
    ...
    private static void sendPacket18(Object nms_playerConnection, Object nms_EntityPlayer, Object nms_Container,
            int windowId, Inventory inventory, String title) throws Exception {
        if (c_PacketOpenWindow == null) {
            c_PacketOpenWindow = findNmsClass("PacketPlayOutOpenWindow").getConstructor(int.class, String.class,
                    findNmsClass("IChatBaseComponent"), int.class);
        }
    ...
    private static String getNmsPackage() {
        if (nms_package == null) { nms_package = "net.minecraft.server." + getNmsVersion(); }

        return nms_package;
    }

    private static String getCrbPackage() {
        if (crb_package == null) { crb_package = "org.bukkit.craftbukkit." + getNmsVersion(); }
